...
class XFeat(nn.Module):
	...
	def __init__(self, coora,fusion,wtconv,scnn,pe,weights = os.path.abspath(os.path.dirname(__file__)) + '/../runs/wtconv/xfeat_default_100000.pth', top_k = 4096, detection_threshold=0.05):
	#def __init__(self, weights = os.path.abspath(os.path.dirname(__file__)) + '/../weights/xfeat.pt', top_k = 4096, detection_threshold=0.05):
		...
		self.net = XFeatModel(coora = coora,fusion = fusion,wtconv = wtconv,scnn=scnn,pe =pe).to(self.dev).eval()
		self.top_k = top_k
